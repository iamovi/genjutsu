...
import { isTextEnglish } from "@/lib/utils";
...
const CommentItem = ({ 
  ...
  const isAlreadyEnglish = useMemo(() => isTextEnglish(comment.content), [comment.content]);
            ...
            <div className="flex items-center gap-1 ml-auto">
              {!isAlreadyEnglish && (
                <button
                  ...
                  className={`p-1 rounded-[3px] hover:bg-secondary transition-colors ${isShowingTranslation ? "text-primary" : "text-muted-foreground"}`}
...
const PostPage = () => {
  ...
  const handleComment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result?.error === "cooldown_active") {
        toast.error(
          `Please wait ${result.retry_after}s before commenting again.`,
        );
        return;
      }
      if (result?.error === "banned") {
        if (result.banned_until) {
          ...
          toast.error(
            `You are banned from commenting until ${until.toLocaleString()}.`,
          );
        } else {
  ...
  const insertMention = (username: string) => {
    ...
    const after = commentText.substring(
      mentionIndex + mentionSearch.length + 1,
    );
  ...
  const handleBookmark = async (id: string, currentlyBookmarked: boolean) => {
    ...
    // Local state sync
    setPost((prev) =>
      prev ? { ...prev, user_bookmarked: !currentlyBookmarked } : null,
    );
  };
  ...
  const pageTitle = post
    ? `${post.profiles?.display_name || "User"}: "${post.content.substring(0, 30)}${post.content.length > 30 ? "..." : ""}" — genjutsu`
    : "Post — genjutsu";
  const pageDesc = post
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <Helmet>
        ...
        <meta
          property="og:image"
          content={post?.profiles?.avatar_url || "/fav.jpg"}
        />
      </Helmet>
      <Navbar />
                    ...
                    {comments.length === 0 ? (
                      <div className="gum-card p-8 text-center bg-secondary/30 border-dashed">
                        <p className="text-muted-foreground text-sm italic">
                          No echoes yet. Silent is the night...
                        </p>
                      </div>
                    ) : (
